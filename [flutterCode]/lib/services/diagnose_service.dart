...
class DiagnoseService {
  ...
  listMatching(
    ...
    Map<String, num> matches = {};
    // ALL DISEASES ASSOCIATED WITH THE SELECTED SYMPTOMS
    List<Disease> allDiseasesWithSelectedSymptoms = await DatabaseService
        .instance
        .getDiseasesBasedOnSelectedSymptoms(pPresentSymptoms);
    for (int i = 0; i < allDiseasesWithSelectedSymptoms.length; i++) {
      List<String> matchingSymptoms =
          allDiseasesWithSelectedSymptoms[i].symptoms.keys.toList();
      for (var element in pPresentSymptoms) {
        print("item: " + element.id);
        matchingSymptoms.removeWhere((item) => item != element.id);
      }
